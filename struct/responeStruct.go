...
type Post struct {
	...
	TimeofPost    string    `json:"time_of_post"` // Use a suitable time format for JSON
...
type Comment struct {
	...
	UserID         int    `json:"user_id"`
	PostID         int    `json:"post_id"`
	TimeofComment  string `json:"time_of_comment"` // Use a suitable time format for JSON
	Message        string `json:"message"`
	LikeCount      int    `json:"like_count"`
	DislikeCount   int    `json:"dislike_count"`
}
...
type Reaction struct {
	...
	TimeofReaction string `json:"time_of_reaction"` // Use a suitable time format for JSON
}
